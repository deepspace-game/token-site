...
function askPrice() {
	...
	GLOBAL $PANCAKESWAP_FACTORY_ADDR_V1;
	GLOBAL $PANCAKESWAP_FACTORY_ADDR_V2;
	GLOBAL $ADDRESS_BNB;
	GLOBAL $BSC_HOST;
	GLOBAL $ADDRESS_USDT;
	GLOBAL $eth;
	...
	try {
	  // Connect to Ganache
		...
		$contractDecimals = hexdec(makeContractCall($riskmoonContract, $contract_address, "decimals")->wait());

		$pairsA =  [
		  getContractPairInfo($PANCAKESWAP_FACTORY_ADDR_V1, $contract_address, $ADDRESS_BNB),
		  getContractPairInfo($PANCAKESWAP_FACTORY_ADDR_V2, $contract_address, $ADDRESS_BNB)
		];
		...
		if($pricet > 0) {
			println("askPrice -> price update success");
			$price = formatTokenStat($pricet, false, $price_decimals);
		} else {
			println("askPrice -> price update failed");
			$price = $price;
		}
	}
	catch (\Exception $exception) {
		println($exception);
		println("pancakeswap - Unable to connect");
	}
}
?>
